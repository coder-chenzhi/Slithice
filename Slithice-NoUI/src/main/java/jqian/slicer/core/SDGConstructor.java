...
public class SDGConstructor implements Runnable{		
	...
	private void loadSootClasses(){
		...
		Global.v().out.println("["+"] Load " + Scene.v().getClasses().size() + 
	...
	private IPtsToQuery doPointsToAnalysis(){
        ...
        if(_options.pointToAnalysis==PointsToAnalysisType.SPARK){
    		...
    		Date endTime = new Date();
    	    Global.v().out.println("["+"] Finish points-to analysis in "+ Utils.getTimeConsumed(startTime,endTime));
        	...
        	throw new RuntimeException("["+"] constructSDG() error: points-to analysis="+ _options.pointToAnalysis + " unsupported.");
        }
	}
	...
	private SDG constructSDG(){
		...
		Global.v().out.println("\n["+"] Constructing system dependence graph(SDG) ... ");   
        ...
        Date endTime = new Date();
        Global.v().out.println("["+"] SDG successfully constructed in "+ Utils.getTimeConsumed(startTime,endTime));
	...
	private void showSDG(SDG sdg){
		...
		Global.v().out.print("\n["+"] Doting SDG file (can be very slow for large SDG): "+filename+" ... ...");
