...
public class TestTypeBasedPtsToAnalysis{
	static void test(){  
		...
		Type t1 = Scene.v().getRefType("test.ptsto.test.ITypeBasedPtsToCase");
		...
		Type t4 = Scene.v().getRefType("test.ptsto.test.ITypeBasedPtsToCase");
    	...
    	for(Iterator<MethodOrMethodContext> it = rm.iterator();it.hasNext();){
    	     ...
    	     //test.PtsToTester.testPtsToQuery(System.out,m,pts2Query);
    	}
	}        
 
	
	public static void main(String[] args) {
		String entryClass = "test.ptsto.test.TypeBasedPtsToCaseImpl";
